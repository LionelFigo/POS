...
$query_total = mysqli_query($koneksi, "SELECT SUM(total_pembelian) AS grand_total FROM nota_beli WHERE tgl_pembelian BETWEEN '$tgl_mulai_esc' AND '$tgl_selesai_esc'") or die(mysqli_error($koneksi));
$data_total  = mysqli_fetch_assoc($query_total);
$grand_total = $data_total['grand_total'] ?? 0;

$ambil_nota_beli = mysqli_query($koneksi, "SELECT * FROM nota_beli WHERE tgl_pembelian BETWEEN '$tgl_mulai_esc' AND '$tgl_selesai_esc' ORDER BY tgl_pembelian DESC") or die(mysqli_error($koneksi));
...
$pdf->Cell(0, 6, 'LAPORAN TOTAL PEMBELIAN', 0, 1, 'C');
$pdf->SetFont('Arial', '', 9);
...
$pdf->Cell(24, 7, 'Tanggal Beli', 1, 0, 'C');
$pdf->Cell(33, 7, 'Total Beli', 1, 0, 'C');
...
if (mysqli_num_rows($ambil_nota_beli) > 0) {
    while ($row = mysqli_fetch_assoc($ambil_nota_beli)) {
        ...
        $pdf->Cell(24, 6, date('d/m/Y', strtotime($row['tgl_pembelian'])), 1, 0, 'C');
        $pdf->Cell(33, 6, 'Rp ' . number_format($row['total_pembelian'], 0, ',', '.'), 1, 0, 'R');
    ...
    $pdf->Cell(190, 8, 'Tidak ada data pembelian pada periode ini.', 1, 1, 'C');
}
...
$pdf->Output('I', 'Laporan_Total_Pembelian_' . $tgl_mulai . '_' . $tgl_selesai . '.pdf');
?>
