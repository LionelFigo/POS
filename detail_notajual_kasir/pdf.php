...
$query_nota = mysqli_query($koneksi, "SELECT * FROM nota_penjualan WHERE kode_nota = '$kode_nota'") or die(mysqli_error($koneksi));
$nota = mysqli_fetch_assoc($query_nota);

if (!$nota) {
    echo '<script>alert("Data nota jual tidak ditemukan!"); window.history.back();</script>';
    exit;
}
...
$query_detail = mysqli_query($koneksi, "SELECT * FROM detail_notajual WHERE kode_nota = '$kode_nota' ORDER BY urut ASC") or die(mysqli_error($koneksi));
...
while ($row = mysqli_fetch_assoc($query_detail)) {
    ...
    $total_harga += (int)$row['total_harga_jual'];
}
...
$pdf->AddPage();
$pdf->Image('../asset_web/img/logo.jpeg', 22, 3, 14);
$pdf->Ln(18);
...
$pdf->Cell(0, 4, 'LAPORAN NOTA JUAL', 0, 1, 'C');
...
$tgl = !empty($nota['tgl_penjualan']) ? date('d/m/Y', strtotime($nota['tgl_penjualan'])) : date('d/m/Y');
...
if (count($items) > 0) {
    foreach ($items as $item) {
        ...
        $qty = $item['jumlah'];
        $harga_satuan = number_format($item['harga_jual'], 0, ',', '.');
        $subtotal = number_format($item['total_harga_jual'], 0, ',', '.');
...
$pdf->Output('I', 'Nota_Jual_' . $kode_nota . '.pdf');
?>
